...
public class RemoveResources implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {

        Player player = (Player) sender;

        if (player.hasPermission("cmd.remresources")) {
            if (args.length == 6) {
                try {
                    ...
                    player.sendMessage(ChatColor.RED + "Usage: /remresources <player> <wood> <wool> <wheat> <clay> <ore>");
                }
            } else {
                player.sendMessage(ChatColor.RED + "Usage: /remresources <player> <wood> <wool> <wheat> <clay> <ore>");
            }
